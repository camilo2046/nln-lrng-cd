...
public class BlogPostDAO {
    ...
    public void likePost(final String permalink, final int ordinal) {
        ...
        if (post != null) 
        {
            ...
            if (ordinal < comments.size ())
            {
                Document postComment = comments.get (ordinal);
                
                if (!postComment.containsKey ("num_likes")) 
                {
                    postComment.put ("num_likes", 0);
                }

                postComment.put ("num_likes", postComment.getInteger ("num_likes") + 1);
            }
